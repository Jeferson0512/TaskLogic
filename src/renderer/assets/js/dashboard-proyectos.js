...
window.DashboardProyectos = (() => {
  ...
  function renderDashboard() {
    ...
    const query = sessionStorage.getItem("projectQuery") || "";
    const filtered = calculated.filter((project) => {
      const text =
        `${project.codigo} ${project.titulo} ${project.responsable} ${project.cliente}`.toLowerCase();
        ...
        <section class="page-header">
          <div class="page-header__grid">
            <div class="page-header__content">
              ...
              <h1 class="page-title">Proyectos con planificador independiente</h1>
          ...
          <div class="panel">
            <div class="panel__head">
              ...
              <div class="project-search">
                ${App.icon("search", 17)}
                <input id="projectSearch" class="input" type="search" value="${App.escapeHtml(query)}" placeholder="Buscar proyecto..." />
              </div>
            </div>

            <div class="project-grid" id="projectGrid">
              ${filtered.length ? filtered.map(projectCardHtml).join("") : emptyProjectsHtml()}
            </div>
          </div>
  ...
  function renderProjectGrid(query = "") {
    ...
    App.bind(grid, "[data-action='edit-project']", "click", (event) => {
      openProjectModal("EDIT", event.currentTarget.dataset.id);
    });

    App.bind(grid, "[data-action='delete-project']", "click", (event) => {
      deleteProject(event.currentTarget.dataset.id);
    });

    App.bind(grid, "[data-action='view-planner']", "click", (event) => {
      goPlanner(event.currentTarget.dataset.id);
    });
  ...
  function bindDashboardEvents() {
    App.bind(app, "[data-action='new-project']", "click", () =>
      openProjectModal("CREATE"),
    );
    App.bind(app, "[data-action='edit-project']", "click", (event) =>
      openProjectModal("EDIT", event.currentTarget.dataset.id),
    );
    App.bind(app, "[data-action='delete-project']", "click", (event) =>
      deleteProject(event.currentTarget.dataset.id),
    );
    App.bind(app, "[data-action='view-planner']", "click", (event) =>
      goPlanner(event.currentTarget.dataset.id),
    );
    App.bind(
      app,
    ...
    App.bind(
      app,
      "[data-action='manage-holidays']",
      "click",
      openHolidaysModal,
    );
    App.bind(
      app,
      "[data-action='close-holidays-modal']",
      "click",
      closeHolidaysModal,
    );
    ...
    if (search) {
      search.addEventListener("input", (event) => {
        ...
        sessionStorage.setItem("projectQuery", value);
        renderProjectGrid(value);
      });
    }
  }
  ...
  async function saveProject() {
    ...
    if (projectModalMode === "CREATE") {
      ...
      App.showToast("Proyecto creado", "Ya puedes abrir su planificador.");
    } else {
      await Api.proyectos.actualizar(cleaned.id, cleaned);
      App.showToast(
        "Proyecto actualizado",
        "Los datos principales fueron guardados.",
      );
    }
  ...
  async function deleteProject(projectId) {
    ...
    App.showToast(
      "Proyecto eliminado",
      "El proyecto fue retirado del listado.",
    );
  }
  ...
  async function addHoliday() {
    ...
    if (!fecha || !nombre) {
      App.showToast("Faltan datos", "Ingresa fecha y nombre del feriado.");
      return;
    }
    ...
    App.showToast("Feriado agregado", "El calendario global fue actualizado.");
  }

  function deleteHoliday(id) {
    state.holidays = state.holidays.filter(
      (holiday) => String(holiday.id) !== String(id),
    );
    persistAndRender();
    App.showToast(
      "Feriado eliminado",
      "Los cronogramas se recalcularán con el nuevo calendario.",
    );
    openHolidaysModal();
  }
  ...
  function resetDemo() {
    ...
    state.selectedProjectId = null;
    persistAndRender();
    App.showToast("Demo reiniciada", "Los datos iniciales fueron restaurados.");
  }
